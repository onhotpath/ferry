...
> A `var` can be assigned to, which the `io.EOF` idiom accepts and [ADR-0016](0016-the-sealed-address-model.md) discloses for `SectionPresent`. It is not accepted here without one more step, because ferry reads its own null on the dump path: an assignment to a published name that core also reads would let one package rewrite every null the walk emits in a process it does not own. So core keeps an unexported copy and reads that, `Null` is a copy handed out once, and the godoc says reassigning it breaks only the reassigning program's own comparisons.
>
...
>
> **A null policy over a registration that declared `.AsMapKey()` is refused at composition.** A key never crosses the boundary as a `Value`: it becomes the segment text of an address, read off whatever the encode half produced. Under a policy that half answers `Null` for a null-ish key, and a `Null` carries no text, so every null-ish key in a map addresses the container's own empty segment - two of them are one address, one entry is lost, and nothing reports it. That is exactly the silent failure `.AsMapKey()` exists to make a registrant think about, so the combination is a refusal rather than a documented corner. A null is about a kind and a key has no kind; the two are structurally incompatible.
>
> **Under a `*T` the pointer's own null wins, in both directions.** A nil pointer writes a null at its own address and a null loads back as a nil pointer, structurally and before any codec is consulted ([ADR-0006](0006-defaults-and-zero-values.md)), so a policied `T` at a `*T` field dumps a null and loads back nil and `load` never runs. The precedence is the right one - the pointer's null is the structural fact and the policy is a codec's opinion - and it was unstated, so it is in the godoc now: pick the pointer or the policy, because the two together are neither.
>
