...
func runW1() {
	...
	fmt.Printf("      prefixFree([/db /db/host])   -> %v\n", errShortW(prefixFree([]Path{path("db"), path("db", "host")})))
	fmt.Printf("      Compile[WPrefix]()          -> %v\n", errShortW(Compile[WPrefix]()))
	...
	fmt.Println("    W0 and W4(f) measure a real hive holding both at once. It does.")
	fmt.Println("    DEFECT FOUND HERE: the inherited prototype's prefixFree checked only")
	fmt.Println("    for exact DUPLICATES, so this schema compiled clean on the branch every")
	fmt.Println("    later ADR was measured against. ADR-0003 is explicit that prefix-")
	fmt.Println("    freeness rather than duplicate detection is the rule, and gives the")
	fmt.Println("    measured reason. Fixed in e_schema.go; the rows above are the fix.")
	fmt.Println("    It was found by asking whether the Registry is MORE permissive than")
	fmt.Println("    core and getting `ok` from core, which is the answer that should have")
	fmt.Println("    been impossible.")
	fmt.Println("    So this is a schema the Registry can represent and core refuses, which")
	fmt.Println("    is the cost ADR-0003 priced as \"a schema nobody writes deliberately\"")
	fmt.Println("    being paid on a plane its own table calls a tree.")
