...
func runE7() {
	...
	fmt.Println("  STRUCTS and are refused, because ADR-0007's chain and ADR-0009's table")
	fmt.Println("  collapse them to a leaf at the empty path. So the rule is asked of the")
	fmt.Println("  COMPILED NODE and not of the Go kind:")
	fmt.Println()
	fmt.Println("    The root must be a struct ferry WALKS, decided after the chain and")
	fmt.Println("    the registry have been asked rather than before.")
	fmt.Println()
	...
	fmt.Println("\n--- E7c: a root map and a root slice are refused too, and E11 is why ---")
	fmt.Println("  An earlier draft let them through, on the grounds that they mint")
	fmt.Println("  non-empty addresses. That is true of a POPULATED one only: a nil or")
	fmt.Println("  empty root map writes Null at its own address, which at the root is the")
	fmt.Println("  empty path, so the permission reopened this very hole. E11 has the")
	fmt.Println("  measurement and the other three costs.")
