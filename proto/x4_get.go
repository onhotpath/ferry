...
func runX4i() {
	...
	fmt.Println("\n  (2) BUT IN WHICH ORDER DOES IT SORT? AS FOUND, ferr_ferrytest.go")
	fmt.Println("      keyed its map by the address's CANONICAL RENDERING and compared")
	fmt.Println("      with strings.Compare, which produced 0 1 10 11 2 3 ... - the order")
	fmt.Println("      ADR-0003 names as the subtle bug, inside one of its own three")
	fmt.Println("      named places. It now keys by the Path and compares with")
	fmt.Println("      CompareSegmentwise:")
	...
	fmt.Println()
	fmt.Println("      FIXED. This was ADR-0003's own named conformance case failing")
	fmt.Println("      inside the helper ADR-0011 describes as reporting `in segment-wise")
	fmt.Println("      order`, so it was in scope of the rule rather than a question")
	fmt.Println("      about it. The repair is the one this probe named: key the map by")
	fmt.Println("      the Path - already comparable - instead of by its String(), and")
	fmt.Println("      compare with CompareSegmentwise. A non-ferry element has no")
	fmt.Println("      address, so it takes the root Path and sorts first.")

	fmt.Println("\n  (3) It was NOT the same defect as the Get sequence, and the two must")
	fmt.Println("      not be conflated: the diff is one of ADR-0003's three NAMED")
	fmt.Println("      places (the conformance suite) and was simply sorting wrongly; a")
	fmt.Println("      Get sequence is none of the three, which is why one was fixed and")
	fmt.Println("      the other became a clause in ADR-0003 saying it is unspecified.")
