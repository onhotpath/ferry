...
func runW3() {
	...
	fmt.Printf("      ferry loading a hive that holds one: %v\n", errShortW(err6))
	fmt.Println("      DEFECT FOUND HERE: until #15 fixed it, the walk DISCARDED a Reader's")
	fmt.Println("      error and substituted Absent, so the driver saying \"I cannot express")
	fmt.Println("      this type\" was indistinguishable from a missing key and the field")
	fmt.Println("      silently took its zero value. That is survey item 5.11's shape inside")
	fmt.Println("      ferry's own walk, and ADR-0001 rules it out by architecture.")
