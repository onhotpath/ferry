...
// root out of a name meant for the other half of the request, and the
// constructor's parameter type is what stops it, before anything runs.
//
// There is no run-time behaviour left to observe, so the assertion has to run
// the compiler over a fixture, exactly as core's own half-pair fixtures do. The
// wording of a diagnostic is Go's rather than ferry's, so what is held is that
// the build fails and that the message names the option's type and the plane
// option type it is not (#338).
...
// mustNotCompile builds one fixture package and holds its refusal to naming both
// plane option types. It is lifted out of the table above so that the table stays
// a table: a subtest body counts against the enclosing function's complexity.
