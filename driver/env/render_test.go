...
func TestTheQuotingIsPinned(t *testing.T) {
	...
	for _, c := range []struct{ name, value, want string }{
		...
		{"a carriage return is double-quoted", "a\rb", `"a\rb"`},
		{"a tab is single-quoted where it can be", "a\tb", "'a\tb'"},
		{"a tab is escaped where double quotes are forced", "it's a\tb", `"it's a\tb"`},
