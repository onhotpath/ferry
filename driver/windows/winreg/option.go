...
//
// Two of them, [WithView] and [Store], and both are [SinkOption]s as well.
//
// Watching is not one of them. It is [Source.Watched], a conversion on the source
// rather than a setting handed to it.
...
//
// The registry is resolved here rather than at the open because opening it is not
// I/O: an implementation records the hive, the subkey and the view, and every call
// it answers opens and closes the key it needs. That is also what makes this
// driver safe to enter from many goroutines with no handle shared between them.
